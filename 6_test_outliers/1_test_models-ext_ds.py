...
BOUNDS_TYPES = ("NOB", "HS")
DATA_SET_IDS = ("ext_ds2", "ext_ds3", "ext_ds4", )
NEX_TS = (5000, 1500, 1500)

DATA_SETS_DIR = os.path.join(BDR, "0_datasets", "containers", "xy_ext_irregular")

for (i, ds_id) in enumerate(DATA_SET_IDS):
    RESULTS_DIR = os.path.join(CSD, f"Test-Results-" + ds_id.upper())
    os.makedirs(RESULTS_DIR, exist_ok=True)
    #
    RVE_MIA = mpi_cnn.load_data_set(
        os.path.join(DATA_SETS_DIR, f"{ds_id}_rve_testing_images.npy"),
        permute_dims=(0, 3, 1, 2),
        merge_chunks=True,
    )
    RVE_META = mpi_cnn.load_data_set(
        os.path.join(DATA_SETS_DIR, f"{ds_id}_rve_testing_labels.npy"),
        merge_chunks=True,
    )
    #
    test_results = {}
    for a_bound_type in BOUNDS_TYPES:
        ...
        for a_model_name in tqdm(os.listdir(MODELS_DIR)):  # NEX_500-R_1
            ...
            #
            nex_ts = NEX_TS[i]
            assert RVE_MIA.shape[0] == nex_ts
            assert RVE_META.shape[0] == nex_ts
